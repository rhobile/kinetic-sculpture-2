...
export function VideoPlayerModal({ image, isOpen, onClose }: VideoPlayerModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl w-[95vw] sm:w-full p-0 overflow-hidden border-none shadow-2xl rounded-none bg-background focus:outline-none flex flex-col max-h-[90vh]">
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <div className="px-4 sm:px-6 py-8 bg-background border-t border-border/10 shrink-0">
            <DialogHeader className="space-y-4 text-left">
              <DialogTitle className="font-normal text-[14pt] tracking-[0.15em] text-foreground uppercase m-0 p-0 leading-tight">
                {image.alt}
              </DialogTitle>
              <DialogDescription className="text-[11pt] sm:text-[12pt] text-muted-foreground font-normal leading-relaxed m-0 p-0 max-w-2xl">
